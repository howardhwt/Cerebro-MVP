 ...
 * 2. Creates/finds company in companys table
...
Return ONLY a JSON object with "company_name" and "needs" array. Example format:
{
  ...
  "needs": [
    ...
    if (existingOrg) {
      ...
      if (orgError || !newOrg) {
        console.error("Error creating company:", orgError);
        return NextResponse.json(
          { error: `Failed to create company: ${orgError?.message || "Unknown error"}` },
          { status: 500 }
        );
      }
      companyId = newOrg.id;
      if (process.env.NODE_ENV === "development") {
        console.log("Created company successfully:", companyId, newOrg);
      }
    }

    // Step 3: Save transcript to calls table
    if (process.env.NODE_ENV === "development") {
      console.log("Saving call to database for org:", companyId);
    }
